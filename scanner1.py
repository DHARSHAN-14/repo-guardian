...
# --- new function to annotate files inline in terminal with red highlight ---
def annotate_file(filepath, matches):
    RED = "\033[91m"
    RESET = "\033[0m"
    try:
    ...
    for i, line in enumerate(lines, start=1):
        hits = [m for m in matches if m.get("line") == i]
        if hits:
            print(f"{RED}{i:4d}: {line.rstrip()}  ⚠️ SECRET DETECTED{RESET}")
        else:
...
def main():
    ...
    for p in paths:
        ...
        if matches:
            ...
            annotate_file(p, matches)  # <-- annotate in terminal
